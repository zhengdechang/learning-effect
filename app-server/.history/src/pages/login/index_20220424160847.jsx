...
export default class Component extends React.PureComponent {
  ...
  async getExamList(filters, current, pageSize) {
    const query = `
  ...
  varifyUserInfo = async (values) => {
    ...
    try {
      ...
      if (user.user_type == 3 && user.mark_id) {
        console.log('await this.getSignList(): ', await this.getSignList());
        let signList = await this.getSignList()?.filter(
          (item) => item.user_id == user?._id,
        );
        let examList = await this.getExamList();
        console.log('signList: ', examList);

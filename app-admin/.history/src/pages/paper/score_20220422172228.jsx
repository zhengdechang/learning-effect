...
class AnswerSheet extends BaseForm {
  ...
  handleSubmit = async () => {
    ...
    answers = answers;
    let sum =
      +answers?.select_score + +answers?.empty_score + +answers?.brief_score;
      ...
      JSON.stringify(
        [
          ...
          },
          {
            ...
            placeholder: '请输入',
            style: { float: 'right' },
            value: this.props.answerList[0]?.sum_score,
            disabled: this.props.answerList[0]?.sum_score && true,
          },
        ]?.filter((item) => !!item),
      ),
    );

    return [choiceColumns, completionColumns, shortColumns, Core];
  };
}
        ...
        render: (text, record, _, action) => {
          ...
          return [
            !!record.sum_score && (
              <a
                onClick={() => {
                  this.props.getPaperId(record?.paper_id);
  ...
  render = () => {
    return (
      ...
      >
        ...
        {this.state.markStatue && (
          <Detail ref={this.paperDetailRef} paperId={this.state.paperId} />
        )}
      </PageContainer>
    );
  };

...
class AnswerSheet extends BaseForm {
  ...
  handleSubmit = async () => {
    ...
    try {
      console.log(this.props.paperId, this.props.examId, '123');
      const { questionList } = await getQuestion(this.props.paperId);

      const { data } = await getExamList({ exam_id: this.props.examId });
      console.log('res: ', data, questionList);

      let knowList = {};

      console.log('list: ', JSON.Parse(data?.[0]?.other));
          ...
          if (key == item._id) {
            item.know.map((i) => {
              knowList = {
                ...knowList,
                [i]:
                  (_.isEmpty(knowList?.[i]) ? knowList?.[i] : 0) +
                  _.divide(+list?.[key], +item.know),
              };
            });
          }
        });

...
export default class Component extends React.PureComponent{
    ...
    formQuestionList = (questionList) => {
      ...
      this.setState((state) => {
        this.props.dispatch({
          type: 'paper/setState',
          payload: {
            surplusScore: state.paper?.paper_score - (choiceScore + completionScore + shortScore),
          }
        });
    ...
    renderQuestion = (list) => {
      return _.map(list, (item, index) => {
        return (<div className={styles.questionItem} key={index}>
          <section>
            <span>{index+1}.</span>&nbsp;
            <span>{item?.question_content?.question_title}</span>&nbsp;
            <span>（{item?.question_score} 分）</span>
            {
              this.state.type === 'edit' &&<Popconfirm
                key="3"
                placement="top"
                title="确认删除此题？"
          ...
          {
            item?.question_content?.options && <ul className={styles.questionOptions}>
              {
                _.map(item?.question_content?.options, (option, idx) => {
                  return (<li key={idx}>{option?.option_key}. {option?.option_value }</li>)
                })
              }
            </ul>
          }
    ...
    renderPaper = () => {
      return (<div className={styles.pageDetail}>
        <header>
          <h1 className={styles.paperTitle}>{this.state.paper?.paper_title}</h1>
          <p className={styles.paperPoints}>考察点：{this.state.paper?.paper_points}</p>
          <p className={styles.paperScore}>总分：{this.state.paper?.paper_score} 分</p>
          <p className={styles.paperTime}>考试时长：{this.state.paper?.paper_time} 分钟</p>
          <p className={styles.paperCreator}>出题人：{this.state.paper?.name}</p>
        </header>

    ...
    let choiceColumns = _.map(
      ...
      (item, index) => {
        ...
        return {
          ...
          disabled: true,
          value: item?.question_content?.answer_value,
        };
      },
    );
    ...
    let choiceScore = _.map(
      ...
      (item, index) => {
        return {
          ...
          value: this.props.answerList[0]?.other?.[`${item?._id}_score`],
          disabled: this.props.isView && true,
        };
      },
    );
...
export default class Component extends React.PureComponent {
  ...
  getAnswerList = async (id, pid, exam_id) => {
    ...
    const transOther = (other) => {
      let res = JSON.parse(other);
      let result = {};
      Object.keys(res).map((item) => {
        result = {
          ...result,
          [item + '_score']: res?.[item],
        };
      });

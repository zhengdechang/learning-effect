...
export default function IndexPage() {
  ...
  const getSign = async () => {
    ...
    try {
      ...
      let data60 = data.filter(item => Number(item.date) > 30 && Number(item.date) <= 60)

      let data90 = data.filter(item => Number(item.date) > 60 && Number(item.date) <= 90)

      let data120 = data.filter(item => Number(item.date) > 90 && Number(item.date) <= 120)

      let data150 = data.filter(item => Number(item.date) > 120 && Number(item.date) <= 150)

      let data180 = data.filter(item => Number(item.date) > 150 && Number(item.date) <= 180)
  ...
  const getScore = async () => {
    ...
    setScoreData(list.sort((a, b) => a['sum_score'] > b['sum_score'] ? 1 : -1))
  };
  ...
  const getDate = async () => {
    ...
    data = data?.map((item, index) => {
      let { paper, ...i } = item;
      return {
        time:moment(Number(i.end_time)).diff(moment(Number(i.start_time)), 'minute'),
        ...paper?.[0],
      };
    });

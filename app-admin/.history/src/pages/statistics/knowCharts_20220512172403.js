        ...
        examList.map(async (item) => {
            ...
            questionList.map(i => {
                Object.keys(JSON.parse(i.know_score)).map(key => {
                    console.log(i.know_score?.[key], _.divide(+i.know_score?.[key], questionList?.length), '2');
                    paperKnow = {
                        ...paperKnow,
                        [key]:
                            (!IsEmpty(paperKnow?.[key]) ? paperKnow?.[key] : 0) +
                            _.divide(+JSON.parse(i.know_score)(i.know_score)?.[key], questionList?.length),
                    };
                })

                console.log('paperKnow: ', paperKnow);

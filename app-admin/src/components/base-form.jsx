    ...
    const renderColumn = (columnOptions, idx) => {
      ...
      return (
        <div key={idx}>
          ...
          {type === 'text.password' && (
            <ProFormText.Password {...columnOptions} />
          )}
          ...
          {type === 'dateTimePicker' && (
            <ProFormDateTimePicker {...columnOptions} />
          )}
          {type === 'dateRangePicker' && (
            <ProFormDateRangePicker {...columnOptions} />
          )}
          {type === 'dateTimeRangePicker' && (
            <ProFormDateTimeRangePicker {...columnOptions} />
          )}
          ...
          {type === 'uploadDragger' && (
            <ProFormUploadDragger {...columnOptions} />
          )}
          {type === 'uploadButton' && (
            <ProFormUploadButton {...columnOptions} />
          )}
          {type === 'dependency' && (
            <ProFormDependency {...columnOptions}>
    ...
    return _.map(columns, (group, idx) => {
      if (_.isArray(group)) {
        ...
        if (_.get(columnOptions, 'type') === 'list') {
          ...
          return (
            <ProFormList key={idx} {...columnOptions}>
              <ProForm.Group>{this.renderFromGroups([children])}</ProForm.Group>
            </ProFormList>
          );
        }
  ...
  renderForm = () => {
    ...
    if (this.props.initialValues || this.state.initialValues) {
      options.initialValues =
        this.props.initialValues || this.state.initialValues;
    ...
    return (
      <>
        {this.state.type === 'default' && (
          <ProForm {...options}>{this.renderFromGroups(columns)}</ProForm>
        )}
        {this.state.type === 'modal' && (
          <ModalForm {...options}>{this.renderFromGroups(columns)}</ModalForm>
        )}
        {this.state.type === 'drawer' && (
          <DrawerForm {...options}>{this.renderFromGroups(columns)}</DrawerForm>
        )}
      </>
    );
  };

...
class AnswerSheet extends BaseForm {
    ...
    formatValue = (obj) => {
        const data = [];
        const questionDict = {};
        _.forEach([...this.props.choiceList, ...this.props.completionList, ...this.props.shortList], (item) => {
    ...
    getColumns = () => {
        ...
        const completionColumns = _.map(this.props.completionList, (item, index) => ({
            type: "text",
            width: 'lg',
            name: `${item?._id}`,
            label: `填空题 ${index + 1}`,
    ...
    render = () => {
        ...
        return (<PageContainer
            header={{
                ...
                extra: [
                    <Alert
                        message={moment(this.state.remainTime).format('MM:DD')}
                        type="warning"
                        showIcon
                    />
                ],
        ...
        >
            {!this.state.examStatus && <ProCard>
                ...
                >
                    <Descriptions.Item label="试卷名称">{this.state.paper?.paper_title}</Descriptions.Item>
                    <Descriptions.Item label="卷面分值">{this.state.paper?.paper_score} 分</Descriptions.Item>
                    <Descriptions.Item label="考试时长">{this.state.paper?.paper_time} 分钟</Descriptions.Item>
                    <Descriptions.Item label="考察知识点" span={4}>
                        {this.state.paper?.paper_points}
                    </Descriptions.Item>
                    <Descriptions.Item label="考试须知" span={4} labelStyle={{ fontWeight: 'bold', color: '#f00' }} contentStyle={{ fontWeight: 'bold', color: '#f00' }}>
